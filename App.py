...
connection = pymysql.connect(
    host='localhost', user='root', password='Kanha@1010', db='sra')
# with connection.cursor() as cur:
#     cur.execute('create database sra;')
...
with open("styles.css") as styles_des:
    st.markdown(f"<style>{styles_des.read()}</style>", unsafe_allow_html=True)
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'dhirrr.k' and ad_password == '12345678':
                st.success("Welcome dhirr")
                # Display Data
